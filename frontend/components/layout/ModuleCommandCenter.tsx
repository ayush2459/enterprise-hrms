...
const CONFIG: Record<string, ModuleConfig> = {
  "/employees": {
    ...
    views: ["Directory", "Analytics", "Departments", "Former employees"],
  ...
  "/recruitment": {
    ...
    views: ["Pipeline", "Openings", "Interviews", "Offers"],
    primary: "New opening",
  },
  "/onboarding": {
    ...
    views: ["Overview", "Checklists", "Documents", "Verification"],
  ...
  "/attendance": {
    ...
    views: ["Today", "Calendar", "Trends", "Exceptions"],
  ...
  "/leaves": {
    ...
    views: ["Employee view", "Requests", "Approvals", "History"],
    primary: "Apply leave",
  },
  "/leave-policies": {
    ...
    views: ["All policies", "Eligibility", "Carry-forward", "Controls"],
  ...
  "/payroll": {
    ...
    views: ["Payroll run", "Employees", "Deductions", "History"],
  ...
  "/performance": {
    ...
    views: ["Overview", "Reviews", "Goals", "Team analytics"],
  ...
  "/documents": {
    ...
    views: ["All documents", "Pending", "Verified", "Expiring"],
  ...
  "/teams": {
    ...
    views: ["Organization", "Teams", "Departments", "Managers"],
    primary: "Create team",
  },
  "/insurance": {
    ...
    views: ["Coverage", "Dependents", "Premiums", "Validity"],
  ...
  "/background-check": {
    ...
    views: ["Verification queue", "Pending", "Cleared", "Exceptions"],
  ...
  "/policies": {
    ...
    views: ["Policy library", "Acknowledgements", "Updates", "Governance"],
  ...
  "/settings": {
    ...
    views: ["Account", "Security", "Access", "System"],
...
export function ModuleCommandCenter() {
  ...
  const key = Object.keys(CONFIG).find((path) => pathname === path || pathname.startsWith(`${path}/`));
  if (!key) return null;
  ...
  return (
    <section className="border-b border-slate-200/80 bg-[#f7f9fd] px-5 pb-4 pt-4 md:px-7">
      <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-r ${config.accent} p-5 text-white shadow-xl shadow-slate-300/20`}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
        {config.metrics.map((metric) => <Metric key={metric.label} item={metric} />)}
      </div>
      ...
      <div className="mt-3 flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm md:flex-row md:items-center">
        <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl bg-slate-50 px-3 py-2">
          ...
          <span className="truncate text-[11px] text-slate-400">Search within {config.title.replace(" command center", "").toLowerCase()}...</span>
          <kbd className="ml-auto hidden rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[9px] text-slate-400 md:block">⌘ K</kbd>
        </div>
        <div className="flex items-center gap-1 overflow-x-auto">
          {config.views.map((view, index) => (
            <button
              key={view}
              type="button"
              onClick={() => index === 0 ? undefined : router.push(`${key}?view=${encodeURIComponent(view.toLowerCase().replaceAll(" ", "-"))}`)}
              className={index === 0 ? "whitespace-nowrap rounded-lg bg-slate-900 px-3 py-2 text-[10px] font-semibold text-white" : "whitespace-nowrap rounded-lg px-3 py-2 text-[10px] font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-800"}
            >
              {view}
            </button>
          ))}
          <button type="button" className="ml-auto hidden shrink-0 items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-[10px] font-semibold text-slate-500 hover:bg-slate-50 md:flex">
