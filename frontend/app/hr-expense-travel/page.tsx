...
export default function HRExpenseTravelPage() {
  ...
  const [settlementTravel, setSettlementTravel] = useState<Travel[]>([]);

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const [queue, setQueue] = useState<"approval" | "settlement">("approval");
  ...
  const load = async () => {
    try {
      ...
      const [
        expenseRows,
        travelRows,
        settlementExpenseRows,
        settlementTravelRows,
      ] = await Promise.all([
        expenseTravelService.hrExpenses(),
        expenseTravelService.hrTravel(),
        expenseTravelService.hrSettlementExpenses(),
        expenseTravelService.hrSettlementTravel(),
      ]);

      setExpenses(expenseRows);
      setTravel(travelRows);
      setSettlementExpenses(settlementExpenseRows);
      setSettlementTravel(settlementTravelRows);
    } catch (err) {
      console.error("Unable to load HR expense/travel queues", err);
      setError("Unable to load the HR expense and travel queues.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const approvalCount = expenses.length + travel.length;
  const settlementCount =
    settlementExpenses.length + settlementTravel.length;

  const visibleExpenses =
    queue === "approval" ? expenses : settlementExpenses;

  const visibleTravel =
    queue === "approval" ? travel : settlementTravel;

  const visibleCount =
    activeTab === "expenses"
      ? visibleExpenses.length
      : visibleTravel.length;
      ...
      <main className="w-full px-5 py-5 md:px-7 lg:px-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-blue-100">
                Review manager-approved requests, verify supporting
                documents, complete final approval and settle employee
                claims.
              </p>
            </div>
        ...
        <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <Kpi
            ...
            value={approvalCount + settlementCount}
          />
        </div>
        ...
        <div className="mb-5 rounded-xl border border-slate-200 bg-white p-1.5">
          <div className="grid grid-cols-2 gap-1">
            ...
            >
              ...
              >
                Manager-approved requests awaiting HR review
              </p>
            ...
            >
              ...
              >
                HR-approved requests awaiting payment
              </p>
        ...
        <div className="mt-3 text-right text-[10px] text-slate-400">
          {visibleCount} request{visibleCount === 1 ? "" : "s"} shown
        </div>
      </main>
        ...
        <div>
          ...
          <p className="mt-0.5 text-[10px] text-slate-400">
            {queue === "approval"
              ...
              : "HR-approved requests awaiting settlement"}
          </p>
        </div>

        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-bold text-slate-500">
          {rows.length} {queue === "approval" ? "pending" : "awaiting settlement"}
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center">
          ...
          <p className="mt-3 text-sm font-semibold text-slate-600">
            {queue === "approval"
              ? "No pending requests"
              : "No requests awaiting settlement"}
          </p>

          <p className="mt-1 text-xs text-slate-400">
            {queue === "approval"
              ? `Manager-approved ${type} requests will appear here.`
              : `HR-approved ${type} requests awaiting settlement will appear here.`}
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-center justify-end">
                  <span className="rounded-lg border border-slate-200 px-3 py-2 text-[10px] font-bold text-slate-600">
                    {queue === "settlement" ? "Settle / Review" : "Review"}
                  </span>
                </div>
              </button>
            );
