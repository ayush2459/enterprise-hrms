...
import type { SeparatedEmployee } from "@/types";
...
export default function FormerEmployeesPage() {
  ...
  const [employees, setEmployees] = useState<SeparatedEmployee[]>([]);
                ...
                {employees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-surface-muted">
                    ...
                    <td className="px-5 py-3 text-gray-600">
                      {emp.separation_date ? new Date(emp.separation_date).toLocaleDateString() : "—"}
                    </td>
                    {isHR && (
                      <td className="px-5 py-3">
                        <Button
